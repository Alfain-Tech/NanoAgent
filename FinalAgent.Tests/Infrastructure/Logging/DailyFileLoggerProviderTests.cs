...
namespace FinalAgent.Tests.Infrastructure.Logging;
...
public sealed class DailyFileLoggerProviderTests : IDisposable
{
    ...
    public DailyFileLoggerProviderTests()
    {
        _tempRoot = Path.Combine(
            Path.GetTempPath(),
            $"FinalAgent-Logs-{Guid.NewGuid():N}");
    ...
    public void CreateLogger_Should_WriteEntriesToDailyLogFile()
    {
        ...
        DailyFileLoggerProvider sut = new(
            ...
            new StubHostEnvironment("FinalAgent"),
            timeProvider);

        ILogger logger = sut.CreateLogger("FinalAgent.Tests.Logging");
        ...
        contents.Should().Contain("info: FinalAgent.Tests.Logging Host startup sequence has begun.");
    }
